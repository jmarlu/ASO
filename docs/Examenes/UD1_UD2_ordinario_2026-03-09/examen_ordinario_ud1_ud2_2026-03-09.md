...
search:
  exclude: true
---
# Examen ordinario UD1 + UD2 (2 horas)


## Contexto de la prueba

Esta prueba ordinaria integra contenidos de:
...
Dentro del contenedor crea la carpeta `~/ordinario_ud1_ud2/` con esta
estructura:

- `~/ordinario_ud1_ud2/scripts/`
- `~/ordinario_ud1_ud2/resultados/`
- `~/ordinario_ud1_ud2/datos/`
- `~/ordinario_ud1_ud2/cron/`
...
1. Genera en el contenedor el fichero
   `~/ordinario_ud1_ud2_entrega.tar.gz` con toda la carpeta de trabajo.
2. Descarga ese fichero al host.
3. Sube a Aules `ordinario_ud1_ud2_entrega.tar.gz`.

## Parte 1 - Preparacion del laboratorio LXD/LXC (1 punto)

1. Crea un contenedor Ubuntu 24.04 llamado `ord-ud1ud2`.
...
5. Guarda en `~/ordinario_ud1_ud2/resultados/lxc.txt` evidencias de:
...
1. Crea `~/ordinario_ud1_ud2/datos/inventario.txt` con este contenido:
...
3. El script debe cumplir todo lo siguiente:
   ...
   - validar que existe `~/ordinario_ud1_ud2/datos/inventario.txt`;
   ...
   - generar `~/ordinario_ud1_ud2/resultados/resumen_inventario.csv`
...
3. Personaliza `/var/www/html/index.html` para que aparezca:
   - el texto `Ordinario UD1 UD2`;
   - el hostname del contenedor.
4. Guarda en `~/ordinario_ud1_ud2/resultados/servicios.txt`:
...
2. Guarda en `~/ordinario_ud1_ud2/resultados/procesos.txt`:
...
6. Copia `estado.service` y `estado.timer` a
   `~/ordinario_ud1_ud2/cron/`.
7. Guarda en `~/ordinario_ud1_ud2/resultados/timer.txt`:
   - el estado actual del timer;
   - la evidencia de su siguiente ejecucion programada;
   - los registros recientes del servicio asociado.
8. Genera `~/ordinario_ud1_ud2_entrega.tar.gz` y preparalo para su descarga
   desde el host.
